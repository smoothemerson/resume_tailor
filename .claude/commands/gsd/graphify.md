...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-sdk query`. Use `node /workspace/.claude/get-shit-done/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-sdk query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node /workspace/.claude/get-shit-done/bin/gsd-tools.cjs config-set graphify.enabled true
...
```bash
node /workspace/.claude/get-shit-done/bin/gsd-tools.cjs graphify query <term>
```
...
```bash
node /workspace/.claude/get-shit-done/bin/gsd-tools.cjs graphify diff
```
...
```bash
node "/workspace/.claude/get-shit-done/bin/gsd-tools.cjs" graphify build
```
...
graphify update . \
  ...
  && node "/workspace/.claude/get-shit-done/bin/gsd-tools.cjs" graphify build snapshot \
  && node "/workspace/.claude/get-shit-done/bin/gsd-tools.cjs" graphify status
```
