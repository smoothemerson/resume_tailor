...
_EMPLOYER_PATTERN = re.compile(r"\\textbf\{([^}]+)\}\\textbf\{\s*\|\s*([^}]+)\}")
    ...
    try:
        ...
        for company, role in original_employers:
            if (company, role) not in tailored_employers:
                logger.warning(
                    f'Employer "{company}" from original resume not found in tailored output.'
                )
        for company, role in tailored_employers:
            if (company, role) not in original_employers:
                logger.warning(
                    f'Employer "{company}" in tailored output was not in original resume — possible hallucination.'
                )
    except Exception as exc:
        logger.warning(f"Employer check failed: {exc}")


def _check_fabricated_technologies(
    original: str, tailored: str, jd_technologies: list | None
) -> None:
...
def _check_technology_substitution(original: str, tailored: str) -> None:
    try:
        ...
        if removed and added:
            logger.warning(
                f"Technology substitution in Skills: removed {sorted(removed)}, added {sorted(added)}"
            )
        elif removed:
            logger.warning(f"Technologies removed from Skills: {sorted(removed)}")
        elif added:
            logger.warning(
                f"Technologies added to Skills not in original: {sorted(added)}"
            )
    except Exception as exc:
    ...
    try:
        ...
        if original_contact_m is not None:
            if tailored_contact_m is None:
                ...
                original_contact_m.group(1).strip()
                != tailored_contact_m.group(1).strip()
            ):
...
def run_guards(
    original_text: str,
    tailored_text: str,
    fences_stripped: bool = False,
    jd_technologies: list | None = None,
) -> None:
